...
struct DetailDateCellView: View {
    @Binding var contact: ContactsModel
    
    var body: some View {

            VStack(alignment: .leading){
                ForEach(0..<contact.date.count,id: \.self){ date in
                    Text(contact.date[date].dateType)
                        .font(.subheadline)
                    Text(contact.date[date].date)
...
struct DetailDateCellView_Previews: PreviewProvider {
